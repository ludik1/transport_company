			...
			modal_link: String() +
				...
				'<form id="redactorInsertLinkForm" method="post" action="">' +
					...
					'<div class="redactor_tab" id="redactor_tab4" style="display: none;">' +
						'<label>Stránka</label><select id="redactor_link_page">' + $('#redactor_pages').html() + '</select>' +
						'<label>Text</label><input type="text" class="redactor_input redactor_link_text" id="redactor_link_page_text" />' +
					'</div>' +
				'</form>' +
				'</div>' +
				'<div id="redactor_modal_footer">' +
	...
	Redactor.prototype = {
		...
		resizeImage: function(resize)
		{
			...
			$(resize).mousemove(function(e)
			{
				if (clicked)
				{
					...
					{
						$(resize).width(new_w);
						$(resize).attr('width', new_w);
					}

					if (new_h > min_h)
					{
						$(resize).height(new_h);
						$(resize).attr('height', new_h);
					}
		...
		imageEdit: function(e)
		{
			...
			var callback = $.proxy(function()
			{
				...
				$('#redactor_form_image_align').val($el.attr('class')); // @JAM
		...
		imageSave: function(el)
		{
			...
			var floatingClass = $('#redactor_form_image_align').val(); // @JAM

			if (floatingClass === 'left')
			{
				$(el).attr('class', 'left');
			}
			else if (floatingClass === 'right')
			{
				$(el).attr('class', 'right');
			}
			else
		...
		showImage: function()
		{
			...
			var callback = $.proxy(function()
			{
				...
				if (this.opts.imageGetJson !== false)
				{
					...
					$.getJSON(this.opts.imageGetJson, data, $.proxy(function(data) { /* pass uploadFields to image getter */
		...
		showLink: function()
		{
			...
			var callback = $.proxy(function()
			{
				...
				else if (turl.search(/\page/) != '-1')
				{
					this.setModalTab(4);

                                        $('#redactor_tab_selected').val(4);
					$('#redactor_link_page').val(turl.match(/\[page=(.*)]/)[1]);
				}
				else
				{
		...
		insertLink: function()
		{
			...
			else if (tab_selected === '3') // anchor
			{
				...
				text = $('#redactor_link_anchor_text').val();
                        ...
                        {
                            link = '[page=' + $('#redactor_link_page').val() + ']';
                            text = $('#redactor_link_page_text').val();
                        }
...
RedactorPlugins.fullscreen = {
	...
	{	
		...
		this.addBtn('fullscreen', 'Celá obrazovka', function(obj)
		{
